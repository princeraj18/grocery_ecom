...
const filteredPartners = partners.filter((partner) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-6 md:p-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Delivery Fleet</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage and monitor your active delivery workforce.</p>
        </div>
  ...
  className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-sm shadow-sm"
  value={searchTerm}
  ...
  {filteredPartners.length} partner(s) found
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-0.5">{partners.length}</h3>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-0.5">{activePartners}</h3>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-0.5">
      ...
      {filteredPartners.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 p-12 text-center">
          <p className="text-slate-500 dark:text-slate-400 font-medium">No delivery partners match your structural scope.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPartners.map((partner) => (
            <div
              ...
              className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col overflow-hidden cursor-pointer"
            >
              {/* Status Header Top Strip */}
              <div className="px-6 pt-6 flex justify-between items-start gap-4">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-full bg-slate-100 overflow-hidden border border-slate-200 dark:border-slate-800 flex-shrink-0">
                  ...
                  <div>
                    <h4 className="font-semibold text-slate-800 dark:text-slate-100 line-clamp-1">{partner.name}</h4>
                ...
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${partner.isAvailable ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-100" }`}>
              ...
              <div className="p-6 flex-grow border-b border-slate-50 space-y-3">
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <span className="font-semibold text-slate-700 dark:text-slate-300">{partner.currentOrders?.length || 0} / {partner.maxOrders || 10} Orders</span>
                </div>
              </div>
